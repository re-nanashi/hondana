...
export class Library implements LibraryImpl {
   ...
   */
  public init = (book: Book, bookStats: Statistics): void => {
    //Event: Display
   ...
   */
  private _displayListFromStorage = ({ createLibraryItem }: any): void => {
    const mangaList: BookData[] = Storage.getBookList();

    mangaList.forEach((manga: BookData): void => {
      this.addBookToList(createLibraryItem(manga));
    });
  };
   ...
   */
  public addBookToList = (data: LibraryItem): void => {
  ...
  private _deleteMangaFromList = (e: MouseEvent): void => {
    ...
    Storage.removeBookFromStorage(title);
